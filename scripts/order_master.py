...
# Set up logging
log_manager = LogManager("order_master")
logger = logging.getLogger(__name__)
logger.info("Starting Category tree data processing script")


logger.info("Loading SQL query from file")
with open('C://Python_scripts//query//order_master.sql', 'r', encoding='utf-8') as file:
    sql_query = file.read()
...
def upload_to_database(df_sql):
    ...
    try:
        ...
        # Verify table exists
        table_exists = verify_table_exists(engine, "order_master", logger)
        if not table_exists:
            logger.error("Target table 'order_master' does not exist")
            raise Exception("Target table 'order_master' does not exist")
        
        # Verify initial table state
        logger.info("Checking initial table state")
        initial_count = verify_table_row_count(engine, "order_master", logger)
        logger.info(f"Initial row count: {initial_count}")
        
        # Truncate existing table using safe method
        logger.info("Starting table truncation")
        truncate_table_safe(engine, "order_master", logger)
        ...
        post_truncate_count = verify_table_row_count(engine, "order_master", logger)
        ...
        final_count = verify_table_row_count(engine, "order_master", logger)
...
if __name__ == '__main__':
    try:
        query_path = 'C://Python_scripts//query//order_master.sql'
        ...
        logger.info("Order Master data processing completed successfully")
        
    except Exception as e:
        logger.error(f"Error in main execution: {str(e)}")
        raise
print("Order Master Updated")
